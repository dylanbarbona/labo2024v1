...
TS_strategy_guantesblancos_202107 <- function( pmyexp, pinputexps, future, final_train, training, validation, testing, pserver="local")
{
  if( -1 == (param_local <- exp_init( pmyexp, pinputexps, pserver ))$resultado ) return( 0 )# linea fija
...
corrida_guantesblancos_202107 <- function(pnombrewf, pvirgen = FALSE) {
  if (-1 == exp_wf_init(pnombrewf, pvirgen)) return(0)
